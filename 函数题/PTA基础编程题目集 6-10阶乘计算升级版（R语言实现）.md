...
函数 Print_Factorial(N):
    如果 N < 0，输出 Invalid input 并返回
    digits ← [1]，按低位到高位保存结果
    对 i 从 2 到 N：
        carry ← 0
        逐位将 digits 乘以 i，并保存当前位和进位
        将剩余进位继续追加到 digits
    逆序拼接 digits 并输出
```
...
- 时间复杂度：`O(ND)`，每个因子都要遍历当前结果的各个数字位。
- 空间复杂度：`O(D)`，`digits` 保存结果的每一位。

### 常见易错点

1. 不能用普通整数或浮点数直接保存 `1000!`，必须按位模拟大数。
2. `digits` 的下标顺序是低位在前，输出时必须先逆序。
3. 每次乘法结束后要继续处理剩余进位，且负数输入应立即输出 `Invalid input`。
