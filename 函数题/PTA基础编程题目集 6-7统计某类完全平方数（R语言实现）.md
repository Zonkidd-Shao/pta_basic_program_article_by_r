...
- 时间复杂度：`O(d)`，平方根判断和数字重复检查均只需处理 N 的位数。
- 空间复杂度：`O(d)`，用于保存拆出的数字字符。

### 常见易错点

1. 仅为完全平方数还不够，还必须存在至少两位相同的数字。
2. 判断平方数时要避免浮点平方根误差，不能只依赖未经修正的 `sqrt`。
3. 数字 0 的各位处理和区间计数要与题目定义保持一致。
