...
- 时间复杂度：`O(log N)`，每次比较都会把候选区间缩小约一半。
- 空间复杂度：`O(1)`，迭代版本只使用左右边界和中间位置。

### 常见易错点

1. 二分查找只适用于严格递增的有序表，不能对无序输入直接使用。
2. 找到元素后返回的是表中位置；R 下标从 1 开始，不能返回 C 语言式的 0 起下标。
3. 区间为空时应返回未找到标志 0，更新 `left` 和 `right` 时要避免死循环。
