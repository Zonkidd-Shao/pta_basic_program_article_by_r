...
```text
函数 Count_Digit(N, D):
    如果 N = 0，根据 D 是否为 0 返回对应结果
    N ← abs(N)
    count ← 0
    当 N ≠ 0 时：
        如果 N 的个位 = D，count 加 1
        N ← N 的整数商 ÷ 10
    返回 count
```
...
- 时间复杂度：`O(d)`，每轮通过取模和整除处理一位数字。
- 空间复杂度：`O(1)`，只使用当前数值、目标数字和计数器。

### 常见易错点

1. 统计负数时只处理绝对值的数字位，负号不属于数字。
2. N 为 0 时，数字 0 应计数 1 次，而其他数字计数为 0。
3. 每轮必须用 `%/% 10` 去掉已处理的个位，否则会死循环。
